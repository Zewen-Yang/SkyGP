...
class LoGGP_rBCM:
    ...
    def predict(self, x):
        ...
        out_var = np.zeros(self.outs, dtype=float)

        models = np.zeros(1000, dtype=int)
        mCount = 1
        models[0] = 0  # start from root
        ...
        for p in range(self.outs):
            ...
            for i in range(mCount):
                ...
                try:
                    v = np.linalg.solve(
                        self.K[p * self.pts : p * self.pts + self.localCount[model],
                            pos * self.pts : pos * self.pts + self.localCount[model]],
                        k_star
                    )
                except np.linalg.LinAlgError:
                    print(f"[PREDICT] Cholesky failed for model {model}, using zeros")
                    v = np.zeros_like(k_star)

                pred_var = self.kernel(x, x, p) - np.dot(k_star, v)
                precisions.append(1.0 / pred_var)
                weighted_means.append(pred_mean / pred_var)

            if M == 0:
                out[p] = 0
                out_var[p] = 1e6
            else:
                # RBCM prior precision (usually prior variance = sigmaF^2)
                sigma0_sq = self.sigmaF[p] ** 2
                prior_precision = 1.0 / sigma0_sq

                # RBCM precision: sum(expert_precision) - (M - 1) * prior_precision
                total_precision = sum(precisions) - (M - 1) * prior_precision
                total_precision = max(total_precision, 1e-3)

                out_var[p] = 1.0 / total_precision

                # RBCM mean: same subtraction of redundant prior effect
                weighted_mean_sum = sum(weighted_means)  # μ_i / σ_i^2
                # Prior mean μ0 assumed to be 0 → term drops out
                out[p] = out_var[p] * weighted_mean_sum
